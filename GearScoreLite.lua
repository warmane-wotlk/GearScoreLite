	...
	if ( CanInspect("mouseover") ) and ( UnitName("mouseover") == Name ) and not ( GS_PlayerIsInCombat ) then 
 	...
 	if ( MouseOverGearScore ) and ( MouseOverGearScore > 0 ) and ( GS_Settings["Player"] == 1 ) then 
		...
		if ( GS_Settings["Level"] == 1 ) then 
			GameTooltip:AddDoubleLine("|cff7fffd4기어스코어 ▶|r "..MouseOverGearScore, "(iLevel: "..MouseOverAverage..")", Red, Green, Blue, Red, Green, Blue)
		else
			GameTooltip:AddLine("|cff7fffd4기어스코어 ▶|r "..MouseOverGearScore, Red, Green, Blue)
		end
	...
	local ItemScore, ItemLevel, EquipLoc, Red, Green, Blue, PVPScore, ItemEquipLoc, enchantPercent = GearScore_GetItemScore(ItemLink);
 	if ( ItemScore >= 0 ) then
		if ( GS_Settings["Item"] == 1 ) then
  			if ( ItemLevel ) and ( GS_Settings["Level"] == 1 ) then Tooltip:AddDoubleLine("|cff7fffd4기어스코어 ▶|r "..ItemScore, "(iLevel "..ItemLevel..")", Red, Blue, Green, Red, Blue, Green);
				...
				Tooltip:AddLine("|cff7fffd4기어스코어 ▶|r "..ItemScore, Red, Blue, Green)
				if ( PlayerEnglishClass == "HUNTER" ) then
					if ( ItemEquipLoc == "INVTYPE_RANGEDRIGHT" ) or ( ItemEquipLoc == "INVTYPE_RANGED" ) then
						Tooltip:AddLine("HunterScore: "..floor(ItemScore * 5.3224), Red, Blue, Green)
					end
...
PersonalGearScore:SetFont("Fonts\\2002.TTF", 12)
...
GearScore2:SetFont("Fonts\\2002.TTF", 12)
GearScore2:SetText("기어스코어")
